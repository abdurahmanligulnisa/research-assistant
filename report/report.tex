...
configurable web-search API in parallel. The headline concurrency result is an
\textbf{8.82$\times$ speedup} over sequential execution measured on live network calls
(N=5 questions, 45.513\,s sequential $\to$ 5.158\,s concurrent); the bottleneck shifts
from network I/O to LLM synthesis once the three source fetches run simultaneously via
...
with a visible warning in \texttt{ResearchResult.sources\_failed}.

The most significant architectural lesson was that parallelising the fetches shifts the
bottleneck to \emph{LLM synthesis time}. Per-phase timing from a live \texttt{DEBUG}
run (see \texttt{artefacts/benchmark\_results.txt}) confirms this directly: the
concurrent fetch phase completes in $\approx$2\,s ($\approx$29\% of total latency),
while \texttt{ai.synthesize()} running in the \texttt{ThreadPoolExecutor}
(\texttt{max\_workers=4}) accounts for $\approx$5\,s ($\approx$74\% of total latency)
per question. Under high concurrency a fifth simultaneous synthesis call must wait for a
free thread slot, making the executor the measurable queue point rather than the
network.
...
\begin{itemize}[itemsep=3pt]
  ...
  \item \textbf{\texttt{src/services/rate\_limiter.py}} --- Two complementary rate limiters.
    \texttt{TokenBucketRateLimiter} controls request frequency per source:
    \texttt{wikipedia\_limiter} (10\,req/s, burst 20),
    \texttt{arxiv\_limiter} (2\,req/s, burst 5),
    \texttt{web\_limiter} (1\,req/s, burst 3).
    \texttt{LlmTpmLimiter} controls LLM token consumption against the provider's
    tokens-per-minute ceiling; \texttt{llm\_tpm\_limiter} defaults to Anthropic's
    free-tier limit (40\,000\,TPM) and is consumed automatically by
    \texttt{AIService.synthesize} using actual prompt~+~completion token counts.
...
The benchmark was run with \texttt{python scripts/benchmark.py --n 5} (live network, API
...
\textbf{Discussion.}
The live speedup (8.82$\times$) significantly exceeds the theoretical $3\times$ maximum
for a single question because \texttt{benchmark.py} pipelines all five questions
through \texttt{asyncio.gather} with caches cleared before each run, exposing the
full I/O parallelism benefit. The offline simulation (2.42$\times$) shows the
scheduling benefit more cleanly because synthesis is replaced by an in-memory stub
that returns instantly. Exact reproduction commands are documented in Appendix~A.

\subsection{Rate limit \& politeness}

The project implements two complementary rate limiters, both in
\texttt{src/services/rate\_limiter.py}, both raising \texttt{RateLimitExceeded}
which is caught by the \texttt{\_TRANSIENT} retry tuple and triggers tenacity's
exponential backoff (1\,s\,$\to$\,2\,s\,$\to$\,4\,s\,$\to$\,8\,s).

\textbf{Request-rate limiter (\texttt{TokenBucketRateLimiter}).}
Each source has a dedicated instance consumed inside \texttt{AIService} before
each network call: Wikipedia at 10\,req/s (burst 20), arXiv at 2\,req/s (burst 5),
and web/DuckDuckGo at 1\,req/s (burst 3). A token bucket was chosen over a
fixed-window counter because fixed windows can allow $2\times$ the quota at window
boundaries, and over a leaky-bucket queue because silently queuing excess requests
increases latency unpredictably.

\textbf{LLM tokens-per-minute limiter (\texttt{LlmTpmLimiter}).}
Provider rate-limit contracts (Anthropic, OpenAI, Google) are expressed in
tokens-per-minute, not requests-per-minute. A single synthesis call that produces
800 completion tokens consumes the same quota as eight 100-token calls; request-rate
limiting alone cannot prevent \texttt{429 RateLimitError} responses from the LLM
provider under heavy load. \texttt{LlmTpmLimiter} uses the same sliding token-bucket
mechanic, but its bucket capacity is \texttt{tpm\_limit} tokens and its refill rate
is \texttt{tpm\_limit / 60} tokens per second (i.e.~the full budget refills in exactly
one minute, matching the provider contract). After each successful
\texttt{AIService.synthesize} call, actual prompt~+~completion token counts are read
from the provider's usage metadata and deducted from the rolling budget; the module
falls back to \texttt{max\_tokens} (1\,024) when usage data is absent, ensuring the
limiter is never silently bypassed. The module-level \texttt{llm\_tpm\_limiter}
defaults to Anthropic's free-tier ceiling (40\,000\,TPM).

If the provider returns HTTP~429 with a \texttt{Retry-After} header, the
